  ...
  return (
    <section className={`relative isolate overflow-hidden bg-[#060707] ${sectionSpace}`}>
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        >
          <p className="font-display text-[0.62rem] font-semibold uppercase tracking-[0.28em] text-brand-accent sm:text-xs sm:tracking-[0.55em]">
            Marques premium
          </p>
        ...
        >
          ...
          <div
            className="premium-logo-marquee relative z-10 flex w-max items-center gap-3 px-3 will-change-transform group-hover:[animation-play-state:paused] sm:gap-7 sm:px-6 md:gap-9"
          ...
          >
            {marqueeBrands.map((brand, index) => (
              <motion.article
                ...
                className={`group/brand relative shrink-0 overflow-hidden rounded-xl bg-[linear-gradient(135deg,rgba(0,161,156,0.62),rgba(255,255,255,0.16)_32%,rgba(0,161,156,0.10)_62%,rgba(255,255,255,0.22))] p-px shadow-[0_18px_70px_rgba(0,0,0,0.45)] transition duration-500 hover:-translate-y-2 hover:shadow-[0_0_60px_rgba(0,161,156,0.28),0_34px_110px_rgba(0,0,0,0.62)] sm:rounded-2xl sm:shadow-[0_28px_100px_rgba(0,0,0,0.52)] ${cardSize}`}
                animate={{ y: [0, -7, 0] }}
                transition={{
                  duration: 5.6,
              ...
              >
                <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-[calc(0.75rem-1px)] border border-white/5 bg-[#101312]/78 px-3 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.11)] backdrop-blur-xl sm:rounded-[calc(1rem-1px)] sm:px-5">
                  ...
                  <img
                    ...
                    draggable={false}
                  />
                  <span className="relative mt-2 font-display text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-brand-silver/78 transition duration-500 group-hover/brand:text-white sm:mt-3 sm:text-sm sm:tracking-[0.24em]">
                    {brand.name}
                  </span>
                  <span className="relative mt-2 h-px w-10 bg-brand-accent/70 shadow-[0_0_18px_rgba(0,161,156,0.8)] transition duration-500 group-hover/brand:w-16 sm:mt-3 sm:w-16 sm:group-hover/brand:w-24" />
                </div>
              </motion.article>
            ))}
          </div>
